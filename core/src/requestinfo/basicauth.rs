...
impl BasicAuth {
    ...
    pub fn from_header(header: &header::HeaderValue) -> Result<Self, RequestInfoError> {
        ...
        let decoded = BASE64.decode(parts.next().ok_or(RequestInfoError::Invalid)?.as_bytes())?;
        let mut credentials = str::from_utf8(&decoded)?.splitn(2, ':');

        let user_id = credentials
            .next()
            .ok_or(RequestInfoError::MissingField("user_id"))
            .map(|user_id| user_id.to_string())?;

        let password = credentials
            .next()
            .ok_or(RequestInfoError::MissingField("password"))
            .map(|password| {
