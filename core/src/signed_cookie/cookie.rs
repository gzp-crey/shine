...
impl CookieSessionInner {
    ...
    fn set_cookie<B>(&self, res: &mut ServiceResponse<B>, state: impl Iterator<Item = (String, String)>) -> Result<(), Error> {
...
mod tests {
    ...
    async fn cookie_session() {
        let mut app = test::init_service(App::new().wrap(CookieSession::signed(&[0; 32]).secure(false)).service(
            web::resource("/").to(|ses: Session| {
                async move {
        ...
        assert!(response.response().cookies().find(|c| c.name() == "actix-session").is_some());
    }

    #[actix_rt::test]
    async fn private_cookie() {
        let mut app = test::init_service(App::new().wrap(CookieSession::private(&[0; 32]).secure(false)).service(
            web::resource("/").to(|ses: Session| {
                async move {
                    let _ = ses.set("counter", 100);
                    "test"
