...
use shine_core::session::{IdentityCookie, SessionKey, UserId, Session};
...
pub async fn login_basicauth(session: Session, auth: BasicAuth, state: web::Data<State>) -> Result<HttpResponse, ActixError> {
    let site = SiteInfo {
        ip: " ".to_string(),
        agent: " ".to_string(),
    };
