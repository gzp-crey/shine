...
use crate::{core::ids::SmallStringId, ecs::ECSError};
...
impl UnsafeResources {
    fn contains(&self, id: &ResourceHandle) -> bool {
    ...
    /// # Safety
    /// Resources which are `!Send` must be retrieved or inserted only on the main thread.
    unsafe fn remove(&mut self, id: &ResourceHandle) -> Option<Box<dyn Resource>> {
        self.map.remove(id).map(|cell| cell.into_inner())
    }

    unsafe fn get<T: Resource>(&self, id: &ResourceHandle) -> Option<ResourceRead<'_, T>> {
        self.map.get(id)?.get::<T>()
    }

    unsafe fn get_mut<T: Resource>(&self, id: &ResourceHandle) -> Option<ResourceWrite<'_, T>> {
        self.map.get(id)?.get_mut::<T>()
    }
}
...
impl Resources {
    ...
    fn get_impl<T: Resource>(&self, id: ResourceId) -> Result<ResourceRead<'_, T>, ECSError> {
        // safety:
        // this type is !Send and !Sync, and so can only be accessed from the thread which
        // owns the resources collection
        let id = ResourceHandle::new::<T>(id);
        Ok(unsafe { self.internal.get::<T>(&id) }.ok_or_else(|| ECSError::ResourceNotFound(id))?)
    }
    ...
    /// # Panics
    /// Panics if the resource is already borrowed mutably.
    pub fn get_with_tag<T: Resource>(&self, tag: &ResourceTag) -> Result<ResourceRead<'_, T>, ECSError> {
    ...
    pub fn get_with_tags<'a, T: Resource, I: IntoIterator<Item = &'a ResourceTag>>(
        &'a self,
        tags: I,
    ) -> Result<TaggedResourceRead<'_, T>, ECSError> {
        let inner = tags
            .into_iter()
            .map(|tag| self.get_with_tag::<T>(tag))
            .collect::<Result<Vec<_>, _>>()?;
        Ok(TaggedResourceRead { inner })
    }

    fn get_mut_impl<T: Resource>(&self, id: ResourceId) -> Result<ResourceWrite<'_, T>, ECSError> {
        // safety:
        // this type is !Send and !Sync, and so can only be accessed from the thread which
        // owns the resources collection
        let id = ResourceHandle::new::<T>(id);
        Ok(unsafe { self.internal.get_mut::<T>(&id) }.ok_or_else(|| ECSError::ResourceNotFound(id))?)
    }
    ...
    /// # Panics
    /// Panics if the resource is already borrowed.
    pub fn get_mut_with_tag<T: Resource>(&self, tag: &ResourceTag) -> Result<ResourceWrite<'_, T>, ECSError> {
    ...
    pub fn get_mut_with_tags<'a, T: Resource, I: IntoIterator<Item = &'a ResourceTag>>(
        &'a self,
        tags: I,
    ) -> Result<TaggedResourceWrite<'_, T>, ECSError> {
        let inner = tags
            .into_iter()
            .map(|tag| self.get_mut_with_tag::<T>(tag))
            .collect::<Result<Vec<_>, _>>()?;
        Ok(TaggedResourceWrite { inner })
    }
}
...
impl<'a> SyncResources<'a> {
    fn get_impl<T: Resource + Sync>(&self, id: ResourceId) -> Result<ResourceRead<'_, T>, ECSError> {
        // safety:
        // this type is !Send and !Sync, and so can only be accessed from the thread which
        // owns the resources collection
        let id = ResourceHandle::new::<T>(id);
    ...
    /// # Panics
    /// Panics if the resource is already borrowed mutably.
    pub fn get<T: Resource + Sync>(&self) -> Result<ResourceRead<'_, T>, ECSError> {
        self.get_impl::<T>(ResourceId::Global)
    }
    ...
    /// # Panics
    /// Panics if the resource is already borrowed mutably.
    pub fn get_with_tag<T: Resource + Sync>(&self, tag: &ResourceTag) -> Result<ResourceRead<'_, T>, ECSError> {
        self.get_impl::<T>(ResourceId::Tag(tag.to_owned()))
    }
    ...
    fn get_mut_impl<T: Resource + Send>(&self, id: ResourceId) -> Result<ResourceWrite<'_, T>, ECSError> {
        // safety:
        // this type is !Send and !Sync, and so can only be accessed from the thread which
        // owns the resources collection
        let id = ResourceHandle::new::<T>(id);
    ...
    /// # Panics
    /// Panics if the resource is already borrowed.
    pub fn get_mut<T: Resource + Send>(&self) -> Result<ResourceWrite<'_, T>, ECSError> {
        self.get_mut_impl::<T>(ResourceId::Global)
    }
    ...
    /// # Panics
    /// Panics if the resource is already borrowed.
    pub fn get_mut_with_tag<T: Resource + Send>(&self, tag: &ResourceTag) -> Result<ResourceWrite<'_, T>, ECSError> {
        self.get_mut_impl::<T>(ResourceId::Tag(tag.to_owned()))
    }
